...
fixedPanelSweepIncrement = 5  # in degrees
sites = []
...
def run_poa_fixed_model(model):
    # get arrays of possible values for parameter sweep
    max_az = 360
    max_tilt = 90
    ...
    # nested for-loop to calculate profiles for each position in the sweep:
    for az in azimuth_sweep:
        for tilt in tilt_sweep:
            ...
            overview_result.format = "CSV"
            overview_result.payload.append("Site ID, Latitude (decimal), Longitude (decimal) ,Total Fixed Tilt"
                                           " Generation [Wh/m2], Winter Fixed Tilt Generation [Wh/m2],"
